-- ═══════════════════════════════════════════════════════════════════════════════
-- UPDATE ONBOARDING LOGIC & CHART TEMPLATES
-- 1. Create Standard/Simple Chart Functions
...
DECLARE
    v_tenant_id UUID;
    v_assets UUID; v_liabilities UUID; v_equity UUID; v_revenue UUID; v_expenses UUID;
BEGIN
    SELECT tenant_id INTO v_tenant_id FROM companies WHERE id = p_company_id;

    -- Root Types (Assumes types exist)
    -- Insert Main Groups
    INSERT INTO chart_of_accounts (tenant_id, company_id, code, name_ar, name_en, type_id, is_group)
    SELECT v_tenant_id, p_company_id, '1', 'الأصول', 'Assets', id, true FROM account_types WHERE code = 'ASSET';
    
    INSERT INTO chart_of_accounts (tenant_id, company_id, code, name_ar, name_en, type_id, is_group)
    SELECT v_tenant_id, p_company_id, '2', 'الخصوم', 'Liabilities', id, true FROM account_types WHERE code = 'LIABILITY';
    
    INSERT INTO chart_of_accounts (tenant_id, company_id, code, name_ar, name_en, type_id, is_group)
    SELECT v_tenant_id, p_company_id, '3', 'حقوق الملكية', 'Equity', id, true FROM account_types WHERE code = 'EQUITY';

    INSERT INTO chart_of_accounts (tenant_id, company_id, code, name_ar, name_en, type_id, is_group)
    SELECT v_tenant_id, p_company_id, '4', 'الإيرادات', 'Revenue', id, true FROM account_types WHERE code = 'REVENUE';
    
    INSERT INTO chart_of_accounts (tenant_id, company_id, code, name_ar, name_en, type_id, is_group)
    SELECT v_tenant_id, p_company_id, '5', 'المصروفات', 'Expenses', id, true FROM account_types WHERE code = 'EXPENSE';
...
BEGIN
    ...
    -- Similar to Fabric but generic
    PERFORM create_simple_chart(p_company_id); -- Start with simple base if needed, or build full.
    -- For now, let's just mark it and maybe add a few extras as placeholder for "Extended"
    -- In a real app, this would have 50+ accounts.
    -- TODO: Expand this. For now, rely on Simple structure but label it Extended.
...
BEGIN
    ...
    IF p_chart_template = 'fabric_extended' AND p_business_type = 'fabric' THEN
