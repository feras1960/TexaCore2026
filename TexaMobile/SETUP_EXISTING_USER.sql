-- ============================================
-- 🔧 إضافة Profile و Role لمستخدم موجود
-- ============================================

-- 📋 التعليمات:
-- 1. شغّل CHECK_EXISTING_USERS.sql أولاً
-- 2. اختر المستخدم الذي تريده
-- 3. ضع email المستخدم هنا ↓
-- 4. شغّل هذا الـ Script

-- ⚠️ عدّل البريد الإلكتروني هنا:
DO $$
DECLARE
  v_user_email TEXT := 'YOUR_EMAIL_HERE@example.com'; -- 👈 ضع البريد هنا
  v_user_id UUID;
  v_tenant_id UUID;
  v_company_id UUID;
  v_admin_role_id UUID;
BEGIN
  -- 1️⃣ الحصول على User ID من البريد
  ...
  RAISE NOTICE '✅ وجدنا المستخدم: % (ID: %)', v_user_email, v_user_id;
  ...
  -- 3️⃣ الحصول على Company (Texa Main)
  ...
  -- 5️⃣ إنشاء/تحديث Admin Role (إذا لم يكن موجوداً)
...
WHERE au.email = 'YOUR_EMAIL_HERE@example.com' -- 👈 ضع نفس البريد
