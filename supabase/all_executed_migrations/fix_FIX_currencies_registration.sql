-- ═══════════════════════════════════════════════════════════════════════════
-- FIX: ربط العملات المختارة بالشركة الافتراضية
-- تاريخ: 2026-02-03
-- الهدف: 
--   1. تحديث register_new_subscriber لتمرير العملات
--   2. إنشاء company_accounting_settings مع العملات المختارة
-- ═══════════════════════════════════════════════════════════════════════════
...
CREATE OR REPLACE FUNCTION register_new_subscriber(
    ...
    p_local_currency VARCHAR(3) DEFAULT NULL       -- العملة المحلية (جديد)
)
RETURNS JSONB AS $$
DECLARE
...
SELECT 
    ...
    '✅ تم التحديث مع دعم العملات' as status
FROM pg_proc 
