...
BEGIN
    ...
    WHERE tenant_id = p_tenant_id
      ...
      AND LENGTH(account_code) = v_parent_length + 1;
    
    IF v_max_code IS NULL THEN
        -- أول حساب فرعي
        v_new_code := p_parent_code || '1';
    ELSE
        -- استخراج الرقم الأخير وزيادته
        v_next_number := CAST(RIGHT(v_max_code, 1) AS INT) + 1;
        IF v_next_number > 9 THEN
            RAISE EXCEPTION 'تم تجاوز الحد الأقصى للحسابات الفرعية تحت %', p_parent_code;
        END IF;
        v_new_code := p_parent_code || v_next_number::TEXT;
    END IF;
