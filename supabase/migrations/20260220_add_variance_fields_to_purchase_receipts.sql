...
-- 1. Add variance tracking fields to purchase_receipts
ALTER TABLE purchase_receipts
    ADD COLUMN IF NOT EXISTS variance_status  TEXT    DEFAULT 'ok'   -- 'ok' | 'requires_review'
        CHECK (variance_status IN ('ok', 'requires_review', 'reviewed', 'approved', 'adjusted')),
    ADD COLUMN IF NOT EXISTS variance_amount  NUMERIC(12,3) DEFAULT 0, -- diff in meters (signed)
    ADD COLUMN IF NOT EXISTS variance_pct     NUMERIC(7,4)  DEFAULT 0, -- diff as %
    ADD COLUMN IF NOT EXISTS variance_tolerance_pct NUMERIC(5,2) DEFAULT 1; -- tolerance used

COMMENT ON COLUMN purchase_receipts.variance_status IS
    'ok=within tolerance, requires_review=out-of-tolerance, reviewed=seen by accountant, approved=accepted, adjusted=credit/debit note created';
COMMENT ON COLUMN purchase_receipts.variance_amount IS
    'Difference in meters: positive=excess, negative=shortage';
COMMENT ON COLUMN purchase_receipts.variance_pct IS
    'Variance as percentage of expected quantity';
COMMENT ON COLUMN purchase_receipts.variance_tolerance_pct IS
    'Tolerance applied at time of receipt (from document or company settings)';
...
COMMENT ON COLUMN purchase_invoices.variance_tolerance_pct IS
    'Optional tolerance override for this invoice. NULL = use company default (1%)';

ALTER TABLE shipment_containers
    ADD COLUMN IF NOT EXISTS variance_tolerance_pct NUMERIC(5,2) DEFAULT NULL;

COMMENT ON COLUMN shipment_containers.variance_tolerance_pct IS
    'Optional tolerance override for this container. NULL = use company default (1%)';
...
COMMENT ON VIEW v_receipt_variance_review IS
    'Shows completed receipts with out-of-tolerance variance, awaiting accountant/manager action';
