-- ══════════════════════════════════════════════════════════════════════
-- 🔧 إصلاح post_sales_invoice() — الترحيل بناءً على المواد المسلّمة فعلياً
-- ══════════════════════════════════════════════════════════════════════
--
-- المشكلة: القيد المحاسبي يُحسب من الفاتورة الأصلية (quantity, total_amount)
--          بدلاً من الكميات المسلّمة فعلياً (delivered_qty)
--
-- الإصلاح:
--   ① الإيرادات = SUM(delivered_qty × unit_price) − الخصم
--   ② الضريبة = إعادة حساب من المبلغ الصافي المسلّم
--   ③ COGS = SUM(cost_price × delivered_qty)
--   ④ تحديث إجمالي الفاتورة ليعكس الأرقام الفعلية
--
-- ══════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- ثالثاً: fallback — حساب ذمم مدينة عام
    IF v_account_ar IS NULL THEN
        SELECT id INTO v_account_ar
        FROM chart_of_accounts
    ...
    -- ② حساب إيرادات المبيعات (4110 → 4100)
    SELECT id INTO v_account_rev
    FROM chart_of_accounts
    WHERE tenant_id = v_trx.tenant_id
      AND (company_id = v_trx.company_id OR company_id IS NULL)
      AND account_code IN ('4110', '4100')
    ORDER BY CASE WHEN account_code = '4110' THEN 0 ELSE 1 END,
             CASE WHEN company_id = v_trx.company_id THEN 0 ELSE 1 END
    LIMIT 1;
    ...
    -- ④ تكلفة البضاعة المباعة COGS (5100 → 5110)
    SELECT id INTO v_account_cogs
    FROM chart_of_accounts
    WHERE tenant_id = v_trx.tenant_id
      AND (company_id = v_trx.company_id OR company_id IS NULL)
      AND account_code IN ('5100', '5110')
    ORDER BY CASE WHEN account_code = '5100' THEN 0 ELSE 1 END,
             CASE WHEN company_id = v_trx.company_id THEN 0 ELSE 1 END
    LIMIT 1;
...
COMMENT ON FUNCTION post_sales_invoice IS 'ترحيل فاتورة مبيعات — قيد محاسبي بناءً على الكميات المسلّمة فعلياً + COGS + حساب عميل فرعي';
