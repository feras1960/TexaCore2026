-- ══════════════════════════════════════════════════════════════════════
-- 🔧 post_sales_invoice() — V3: COGS بالعملة الأساسية + قيد منفصل
-- ══════════════════════════════════════════════════════════════════════
--
-- V3 — مارس 30, 2026:
-- ✅ الإيرادات تُحسب من delivered_qty بدلاً من quantity
-- ✅ الضريبة تُعاد حسابها من الصافي المسلّم
-- ✅ COGS تُحسب من cost_price × delivered_qty
-- ✅ 🆕 قيد COGS منفصل بالعملة الأساسية (USD) مع exchange_rate = 1
-- ✅ 🆕 ربط cogs_journal_entry_id بالفاتورة
-- ✅ Fallback للفاتورة الأصلية إذا لم يكن هناك تسليم
-- ✅ 🆕 Cascade delete via trg_cleanup_sales_journal trigger
--
-- المشكلة المُصلحة:
-- ❌ V2: قيد COGS يحمل عملة الفاتورة (UAH) لكن المبلغ بالدولار (USD)
--    → get_all_account_balances_fc يقسم المبلغ على سعر الصرف → فرق في الشجرة
-- ✅ V3: قيد COGS بالعملة الأساسية (من companies.default_currency) مع exchange_rate = 1
-- ══════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    --    🔑 العملة = العملة الأساسية (USD) وليس عملة الفاتورة  
    --    لأن cost_price مخزّن بالدولار (من cost_per_meter من فاتورة الشراء)
    -- ════════════════════════════════════════════════════════
    IF v_cost_amount > 0 AND v_account_cogs IS NOT NULL AND v_account_inv IS NOT NULL THEN
        ...
        INSERT INTO journal_entries (
            ...
            v_base_currency, 1,  -- 🔑 العملة الأساسية + سعر صرف 1
            'draft', false, v_user_id,
