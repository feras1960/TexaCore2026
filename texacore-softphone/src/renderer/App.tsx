  ...
  useEffect(() => {
    const handleWebCall = (e: any) => {
      ...
      if (callState === 'idle') {
        ...
        const callerInfo = data.visitor_id ? `Web-${data.visitor_id.substring(0,6)}` : 'زائر ويب';
        setActiveNumber(callerInfo);
        setActiveTab('dialpad');
  ...
  return (
    <div className="app-container">
      ...
      {callState !== 'idle' && isCallMinimized && (
        <div className="active-call-banner" onClick={() => setIsCallMinimized(false)}>
          <div className="banner-content">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 00-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>
            <span>مكالمة جارية: {activeNumber} ({formatTime(callDuration)})</span>
          </div>
          <span>عودة</span>
        </div>
      )}
      ...
      <main className="main-content">
        {activeTab === 'dialpad' && callState === 'idle' && (
        ...
        {callState !== 'idle' && !isCallMinimized && (
          <div className="call-screen">
            ...
            <h2 className="caller-number">{activeNumber}</h2>
        ...
        {activeTab === 'history' && callState === 'idle' && (
          <CallHistory onCall={(n) => { setTarget(n); setActiveTab('dialpad'); setTimeout(() => makeCall(n), 100); }} />
        )}
        {activeTab === 'contacts' && callState === 'idle' && (
          <Contacts onCall={(n) => { setTarget(n); setActiveTab('dialpad'); setTimeout(() => makeCall(n), 100); }} />
        )}
        {activeTab === 'settings' && callState === 'idle' && (
