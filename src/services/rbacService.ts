...
class RBACService {
    ...
    async assignRoleToUser(data: AssignRoleDTO): Promise<UserRole> {
        ...
        const [{ data: profile }, { data: role }] = await Promise.all([
            supabase.from('user_profiles').select('tenant_id').eq('id', data.user_id).single(),
    ...
    async getUserVisibleModules(userId: string): Promise<string[]> {
        try {
            ...
            // Get user's company brand for module filtering
            const { data: profile } = await supabase
                .from('user_profiles')
                .select('company_id')
                .eq('id', userId)
