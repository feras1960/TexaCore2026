...
export async function generateVariants(
    ...
    try {
        ...
        // 0. جلب بيانات المحاور (الأسماء) من variant_axes
        const axisIds = axes.map(a => a.axis_id);
        const { data: axisMetadata } = await supabase
            .from('variant_axes')
            .select('id, name_ar, name_en')
            .in('id', axisIds);
        const axisNameMap = new Map<string, { name_ar: string; name_en: string }>();
        (axisMetadata || []).forEach((a: any) => axisNameMap.set(a.id, { name_ar: a.name_ar, name_en: a.name_en }));
        ...
        // 4. جلب المتغيرات الموجودة لمنع التكرار (كل الـ tenant وليس المنتج فقط)
        const existing = await getProductVariants(product_id);
        const existingSKUs = new Set(existing.map(v => v.sku));
        
        // 4b. فحص SKUs في كل الـ tenant لمنع انتهاك unique constraint
        const { data: allTenantSKUs } = await supabase
            .from('product_variants')
            .select('sku')
            .eq('tenant_id', tenantId);
        const globalSKUs = new Set((allTenantSKUs || []).map((v: any) => v.sku));
        ...
        for (const combo of combinations) {
            ...
            let sku = generateVariantSKU(parent_code, valueCodes);

            // تخطي إذا موجود لنفس المنتج
            if (existingSKUs.has(sku)) {
                skipped++;
                continue;
            }
            
            // إذا SKU موجود في tenant آخر — أضف suffix رقمي
            if (globalSKUs.has(sku)) {
                let suffix = 2;
                while (globalSKUs.has(`${sku}-${suffix}`)) suffix++;
                sku = `${sku}-${suffix}`;
            }
            ...
            // بناء variant_data JSONB — أسماء المحاور وقيمها (للتجميع البصري في الشجرة)
            const variantDataObj: Record<string, any> = {};
            for (let i = 0; i < combo.length; i++) {
                const c = combo[i];
                // نحتاج اسم المحور → نجلبه من axisNameMap
                const axisNames = axisNameMap.get(c.axis_id);
                const axisKey = c.axis_id; // نستخدم axis_id كمفتاح
                variantDataObj[axisKey] = {
                    value_id: c.value.id,
                    name_ar: c.value.name_ar,
                    name_en: c.value.name_en || c.value.code,
                    code: c.value.code,
                    color_hex: c.value.hex_code || null,
                    axis_name_ar: axisNames?.name_ar || '',
                    axis_name_en: axisNames?.name_en || '',
                    sort_order: i, // ← ترتيب المحور (0 = أساسي/التصميم)
                };
            }
            ...
            globalSKUs.add(sku); // منع تكرار في نفس الدفعة
        }

        // 5b. إنشاء مواد فرعية في fabric_materials (بدون مجموعات إضافية)
        if ((product_table || 'fabric_materials') === 'fabric_materials' && created.length > 0) {
            ...
            const { data: parentMat } = await supabase
                ...
                .select('name_ar, name_en, group_id, category, unit, composition, currency, purchase_price, selling_price')
                .eq('id', product_id)
                .single();

            if (parentMat) {
                for (const variant of created) {
                    // استخراج color_hex من variant_data إن وُجد
                    const vData = variant.variant_data || {};
                    let colorHex: string | null = null;
                    for (const key of Object.keys(vData)) {
                        if (vData[key]?.color_hex) {
                            colorHex = vData[key].color_hex;
                            break;
                        }
                    }

                    // إنشاء مادة فرعية — ترث خصائص الأم + تُربط عبر parent_material_id
                    await supabase
                        .from('fabric_materials')
                        .insert({
                            tenant_id: tenantId,
                            ...
                            name_en: (() => {
                                const parentName = parentMat.name_en || parentMat.name_ar || '';
                                const variantName = variant.name_en || variant.sku || '';
                                return variantName ? `${parentName} - ${variantName}`.replace(/^\s*-\s*/, '') : parentName;
                            })(),
                            group_id: parentMat.group_id,     // ← نفس مجموعة الأم (لا مجموعة جديدة!)
                            category: parentMat.category || 'woven',
                            unit: parentMat.unit || 'meter',
                            composition: parentMat.composition || null,
                            currency: parentMat.currency || null,
                            purchase_price: parentMat.purchase_price || null,
                            selling_price: parentMat.selling_price || null,
                            status: 'active',
                            has_variants: false,
                            is_variant_parent: false,
                            parent_material_id: product_id,   // ← ربط مباشر بالأم
                            variant_id: variant.id,           // ← ربط بالمتغير
                            variant_data: variant.variant_data || {},  // ← بيانات المحاور للتجميع البصري
                            custom_fields: {
