...
export interface Container {
  ...
  // معلومات الشحن
  origin_country?: string; // max 3 chars (ISO code)
  origin_port?: string;
...
export async function getContainerById(containerId: string) {
  const { data, error } = await supabase
    ...
    .select(`
      *,
      supplier:suppliers(id, name_ar, name_en, code, country),
...
export async function calculateLandedCost(containerId: string) {
  ...
  if (!container) throw new Error('الكونتينر غير موجود');
  
  // جلب البنود
  const items = container.items || [];
  const expenses = container.expenses || [];
  
  // حساب إجمالي قيمة البضاعة
  const totalGoodsValue = items.reduce((sum: number, item: ContainerItem) => {
    return sum + (item.unit_cost * item.expected_quantity);
  }, 0);
  
  // حساب إجمالي المصاريف
  const totalExpenses = expenses.reduce((sum: number, exp: ContainerExpense) => {
