...
export default function TrialBalance() {
  ...
  const reportData: ReportData = {
    ...
    type: 'table',
    summaryItems: [
      { label: language === 'ar' ? 'إجمالي المدين' : 'Total Debit', value: `SAR ${totalDebit.toLocaleString()}`, type: 'positive' },
      { label: language === 'ar' ? 'إجمالي الدائن' : 'Total Credit', value: `SAR ${totalCredit.toLocaleString()}`, type: 'negative' },
      { label: language === 'ar' ? 'الفرق' : 'Difference', value: `SAR ${Math.abs(totalDebit - totalCredit).toLocaleString()}`, type: totalDebit === totalCredit ? 'positive' : 'negative' },
    ...
    rows: data.map(row => [
      row.code,
      row.account,
      row.debit > 0 ? `SAR ${row.debit.toLocaleString()}` : '-',
      row.credit > 0 ? `SAR ${row.credit.toLocaleString()}` : '-',
    ]),
    totals: [
      { label: language === 'ar' ? 'إجمالي المدين' : 'Total Debit', value: `SAR ${totalDebit.toLocaleString()}`, highlight: false },
      { label: language === 'ar' ? 'إجمالي الدائن' : 'Total Credit', value: `SAR ${totalCredit.toLocaleString()}`, highlight: true },
    ],
  };
