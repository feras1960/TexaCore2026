...
export default function NewJournalEntrySheet({
    ...
    const isLinkedEntry = Boolean(
        ...
        ['container', 'purchase_invoice', 'sales_invoice', 'goods_receipt', 'remittance'].includes(entryData.reference_type)
    );
    const linkedEntryTypes = ['container_expense', 'container_expense_reversal', 'purchase', 'auto', 'provisional', 'remittance'];
    ...
    const getProtectedSource = () => {
        ...
        if (entryData?.reference_type === 'remittance' || entryData?.entry_type === 'remittance') return language === 'ar' ? 'حوالة' : 'Remittance';
