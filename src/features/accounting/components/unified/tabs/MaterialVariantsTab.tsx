...
export function MaterialVariantsTab({ data, mode, onChange }: MaterialVariantsTabProps) {
    ...
    // ─── Load existing variant selections for variant parents ───
    // ⚠️ يجب أن تكون الـ hooks قبل أي early return
    const [existingVariantData, setExistingVariantData] = useState<any>(null);
    const [loadingExisting, setLoadingExisting] = useState(false);
    
    useEffect(() => {
        if (!materialId || !data?.is_variant_parent) return;
        
        const loadExistingSelections = async () => {
            setLoadingExisting(true);
            try {
                const { supabase } = await import('@/lib/supabase');
                // جلب المواد الفرعية مع variant_data
                ...
                if (children && children.length > 0) {
                    // استخراج المحاور والقيم المُستخدمة
                    const enabledAxisIds = new Set<string>();
                    const selectedValues: Record<string, string[]> = {};
                    let hierarchicalAxisId: string | null = null;
                    
                    for (const child of children) {
                        if (!child.variant_data) continue;
                        const vd = child.variant_data as Record<string, any>;
                        const axisKeys = Object.keys(vd);
                        
                        for (const axisId of axisKeys) {
                            enabledAxisIds.add(axisId);
                            const valueId = vd[axisId]?.value_id;
                            if (valueId) {
                                if (!selectedValues[axisId]) selectedValues[axisId] = [];
                                if (!selectedValues[axisId].includes(valueId)) {
                                    selectedValues[axisId].push(valueId);
                                }
                            }
                        }
                    }
                    
                    // اكتشاف المحور الهرمي (الثاني بعد الأساسي)
                    if (enabledAxisIds.size >= 2) {
                        const firstChild = children.find(c => c.variant_data && Object.keys(c.variant_data).length >= 2);
                        if (firstChild) {
                            const vd = firstChild.variant_data as Record<string, any>;
                            const entries = Object.entries(vd).sort(([, a]: any, [, b]: any) => (a.sort_order || 0) - (b.sort_order || 0));
                            if (entries.length >= 2) {
                                hierarchicalAxisId = entries[1][0]; // المحور الثاني = هرمي
                            }
                        }
                    }
                    
                    setExistingVariantData({
                        _variant_selections: {
                            enabledAxisIds: Array.from(enabledAxisIds),
                            selectedValues,
                            hierarchicalAxisId,
                        },
                ...
                console.warn('Failed to load existing variant config:', err);
            } finally {
    ...
    }, [materialId, data?.is_variant_parent]);

    // ─── Loading ───
    if (axesLoading || loadingExisting) {
        return (
    ...
    // ─── Create Mode (no materialId yet) — اختيار المحاور والقيم مباشرة ───
    // أو المادة الأم الموجودة (is_variant_parent) — عرض الاختيارات المحمّلة
    if (!materialId || (data?.is_variant_parent && materialId)) {
        return (
            <CreateModeVariants
                axes={axes}
                isAr={isAr}
                ...
                onChange={onChange}
                initialData={existingVariantData || data}
            />
        );
    }
...
// ═══════════════════════════════════════════════════════════════
// 🆕 Create Mode — اختيار المحاور والقيم مباشرة عند إنشاء مادة جديدة
// ═══════════════════════════════════════════════════════════════
function CreateModeVariants({ axes, isAr, isReadOnly, onChange, initialData }: {
    axes: VariantAxis[];
    isAr: boolean;
    isReadOnly: boolean;
    onChange?: (updates: any) => void;
    ...
    // State: axes enabled + selected values per axis
    const [enabledAxisIds, setEnabledAxisIds] = useState<Set<string>>(() => {
        // Restore from initialData if available
        const saved = initialData?._variant_selections?.enabledAxisIds;
        return saved ? new Set(saved) : new Set();
    });
    const [selectedValues, setSelectedValues] = useState<Record<string, string[]>>(() => {
        return initialData?._variant_selections?.selectedValues || {};
    });
    const [hierarchicalAxisId, setHierarchicalAxisId] = useState<string | null>(() => {
        return initialData?._variant_selections?.hierarchicalAxisId || null;
    });

    // Notify parent of changes
    const notifyParent = useCallback((
        newEnabled: Set<string>, 
        newSelected: Record<string, string[]>,
        newHierarchical: string | null
    ) => {
        if (!onChange) return;
        const hasVariants = newEnabled.size > 0 && Object.values(newSelected).some(v => v.length > 0);
        
        // Build axes config for generateVariants
        const axesConfig = Array.from(newEnabled)
            .filter(axisId => (newSelected[axisId] || []).length > 0)
            .map(axisId => ({
                axis_id: axisId,
                value_ids: newSelected[axisId] || [],
                is_hierarchical: axisId === newHierarchical,
                parent_axis_id: newHierarchical === axisId 
                    ? Array.from(newEnabled).find(id => id !== axisId) || undefined
                    : undefined,
            }));

        onChange({ 
            has_variants: hasVariants,
            is_variant_parent: hasVariants,
            _variant_selections: {
                enabledAxisIds: Array.from(newEnabled),
                selectedValues: newSelected,
                hierarchicalAxisId: newHierarchical,
                axesConfig,
            }
        });
    }, [onChange]);

    const handleToggleAxis = useCallback((axisId: string, enabled: boolean) => {
        const newSet = new Set(enabledAxisIds);
        if (enabled) {
            newSet.add(axisId);
        } else {
            newSet.delete(axisId);
            // Remove selected values for this axis
            const newSelected = { ...selectedValues };
            delete newSelected[axisId];
            setSelectedValues(newSelected);
            // Clear hierarchical if removed
            if (hierarchicalAxisId === axisId) {
                setHierarchicalAxisId(null);
                notifyParent(newSet, newSelected, null);
                setEnabledAxisIds(newSet);
                return;
            }
            notifyParent(newSet, newSelected, hierarchicalAxisId);
            setEnabledAxisIds(newSet);
            return;
        }
        setEnabledAxisIds(newSet);
        notifyParent(newSet, selectedValues, hierarchicalAxisId);
    }, [enabledAxisIds, selectedValues, hierarchicalAxisId, notifyParent]);

    const handleValuesChange = useCallback((axisId: string, vals: string[]) => {
        const newSelected = { ...selectedValues, [axisId]: vals };
        setSelectedValues(newSelected);
        notifyParent(enabledAxisIds, newSelected, hierarchicalAxisId);
    }, [selectedValues, enabledAxisIds, hierarchicalAxisId, notifyParent]);

    const handleHierarchical = useCallback((axisId: string, isHier: boolean) => {
        const newHier = isHier ? axisId : null;
        setHierarchicalAxisId(newHier);
        notifyParent(enabledAxisIds, selectedValues, newHier);
    }, [enabledAxisIds, selectedValues, notifyParent]);

    // Count total combinations
    const totalCombinations = useMemo(() => {
        const nonEmpty = Array.from(enabledAxisIds)
            .map(id => (selectedValues[id] || []).length)
            .filter(n => n > 0);
        if (nonEmpty.length === 0) return 0;
        return nonEmpty.reduce((acc, n) => acc * n, 1);
    }, [enabledAxisIds, selectedValues]);

    return (
        <div className="space-y-5 pb-6">
            <InfoBanner isAr={isAr} />

            {/* Axis Toggles with inline value selection */}
            <Card>
                <CardHeader className="pb-3">
                    <CardTitle className="flex items-center gap-2 text-base">
                        <Layers className="w-5 h-5 text-erp-primary" />
                        {isAr ? 'اختر المحاور والقيم' : 'Select Axes & Values'}
                    </CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                    {axes.filter(a => a.is_active).sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0)).map(axis => {
                        const isEnabled = enabledAxisIds.has(axis.id);
                        ...
                        return (
                            <div key={axis.id} className={cn(
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <Switch
                                        checked={isEnabled}
                                        onCheckedChange={(checked) => handleToggleAxis(axis.id, checked)}
                                        disabled={isReadOnly}
                                    />
                                </div>

                                {/* Inline Value Selector (when enabled) */}
                                {isEnabled && (
                                    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                                        <AxisValueSelector
                                            axisId={axis.id}
                                            axis={axis}
                                            selected={selectedValues[axis.id] || []}
                                            onChange={(vals) => handleValuesChange(axis.id, vals)}
                                            isAr={isAr}
                                        />

                                        {/* Hierarchical option */}
                                        {enabledAxisIds.size >= 2 && (() => {
                                            const otherAxisId = Array.from(enabledAxisIds).find(id => id !== axis.id);
                                            const otherAxis = axes.find(a => a.id === otherAxisId);
                                            const thisIsHierarchical = hierarchicalAxisId === axis.id;
                                            const anotherIsHierarchical = hierarchicalAxisId !== null && hierarchicalAxisId !== axis.id;

                                            return (
                                                <div className="mt-2 pt-2 border-t border-gray-50 dark:border-gray-800">
                                                    <div className="flex items-center gap-2">
                                                        <Checkbox
                                                            id={`hier-create-${axis.id}`}
                                                            checked={thisIsHierarchical}
                                                            onCheckedChange={(checked) => handleHierarchical(axis.id, !!checked)}
                                                            disabled={isReadOnly || (anotherIsHierarchical && !thisIsHierarchical)}
                                                        />
                                                        <Label htmlFor={`hier-create-${axis.id}`} className="text-xs text-gray-600 dark:text-gray-400">
                                                            {isAr
                                                                ? `تجميع حسب ${isAr ? otherAxis?.name_ar : otherAxis?.name_en} ← ${axis.name_ar} كفرعي`
                                                                : `Group by ${otherAxis?.name_en} ← ${axis.name_en} as child`}
                                                        </Label>
                                                    </div>
                                                </div>
                                            );
                ...
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg p-4 border border-purple-200 dark:border-purple-800">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-purple-700 dark:text-purple-400">
                                {isAr ? 'عند الحفظ سيتم إنشاء:' : 'On save will create:'}
                            </span>
                        </div>
                    ...
                    <p className="text-xs text-purple-600 dark:text-purple-400 mt-2">
                        {isAr
                            ? `سيتم إنشاء المادة الأم + ${totalCombinations} مادة فرعية (متغيرات) دفعة واحدة.`
                            : `The parent material + ${totalCombinations} child materials (variants) will be created in one shot.`}
                    </p>
                </div>
            )}
        </div>
