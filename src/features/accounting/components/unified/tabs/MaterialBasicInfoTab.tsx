...
export function MaterialBasicInfoTab({ data, mode, onChange, groups = [] }: MaterialBasicInfoTabProps) {
    ...
    return (
        <div className="space-y-6 pb-6">
            ...
            {/* Primary Name Section — based on user's language */}
            <Card>
                <CardHeader className="pb-3">
                    <CardTitle className="flex items-center gap-2 text-lg">
                ...
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Primary Name — user's language */}
                        <div className="space-y-2">
                            <Label htmlFor={`name_${language}`} className="text-sm font-medium">
                                {(() => {
                                    const userLang = supportedLanguages.find(l => l.code === language);
                                    return userLang ? userLang.label : 'Name';
                                })()} <span className="text-red-500">*</span>
                            </Label>
                            <Input
                                id={`name_${language}`}
                                value={data?.[`name_${language}`] || ''}
                                onChange={(e) => handleChange(`name_${language}`, e.target.value)}
                        ...
                        {/* English Name (secondary) — hidden if user language is already English */}
                        {language !== 'en' && (
                            <div className="space-y-2">
            ...
            <Card>
                <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                        ...
                        {!isReadOnly && (
                            <TranslateButton
                                sourceText={data?.[`name_${language}`] || data?.name_en || data?.name_ar || ''}
                ...
                <CardContent className="space-y-3">
                    {supportedLanguages
                        .filter(l => l.code !== language && l.code !== 'en')
                        .map((lang) => (
