...
export interface ReceiptVoucherTabProps {
    data: any;
    mode: SheetMode;
...
export function ReceiptVoucherTab({
        ...
        if (loadedIdRef.current !== incomingId) {
            if (data && (mode === 'edit' || mode === 'view')) {
                ...
                const loadedLines = data.lines?.map((line: any) => ({
                    ...
                    debit: 0,
                    credit: Number(line.credit) || 0, // In Receipt, lines are credit
    ...
    const totals = useMemo(() => {
        return lines.reduce((acc, row) => ({
            amount: acc.amount + (Number(row.credit) || 0),
        }), { amount: 0 });
    }, [lines]);
    ...
    const editableColumns = useMemo(() => [
        ...
        { key: 'account_id', type: 'account' as const, required: true },
        {
            key: 'link_type', type: 'select' as const, options: [
                ...
                { value: 'employee', label: t('accounting.linkTypes.employee') },
                { value: 'project', label: t('accounting.linkTypes.project') }
            ]
        },
        ...
        <div className="flex flex-col gap-4 p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <Card className="p-3 flex flex-col items-center justify-center gap-1.5 border shadow-sm bg-gray-50">
                    <span className="font-bold text-sm text-green-700 capitalize">{t('accounting.receipt.title')}</span>
            ...
            <div className="border rounded-xl shadow-sm bg-white dark:bg-gray-800 overflow-hidden">
                <NexaDataTable
                    ...
                    columns={columns}
                    isRTL={isRTL}
                    persistKey="accounting-receipt-lines-v5"
                    ...
                    creditKey="credit" // Only show credit total
