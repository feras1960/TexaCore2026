...
export function MaterialInventoryTab({ data, onClose, onOpenRoll }: MaterialInventoryTabProps) {
    ...
    const inventoryData = inventoryQuery.data?.stockData || [];
    const allWarehouses = inventoryQuery.data?.warehouses || [];
    ...
    const getStockStatus = useCallback((record: InventoryRecord): 'in_stock' | 'low_stock' | 'out_of_stock' => {
        const effectiveTotal = record.total_length + (record.loose_stock || 0);
        if (effectiveTotal <= 0) return 'out_of_stock';
        const minStock = data?.min_stock || 0;
        if (minStock > 0 && effectiveTotal <= minStock) return 'low_stock';
        const effectiveAvailable = record.available_length + (record.loose_stock || 0);
        if (effectiveAvailable <= 0 && record.reserved_length > 0) return 'low_stock';
        return 'in_stock';
    }, [data?.min_stock]);
    ...
    const totals = useMemo(() => inventoryData.reduce(
        (acc, record) => ({
            ...
            loose: acc.loose + (record.loose_stock || 0),
        }),
        { rolls: 0, total: 0, available: 0, reserved: 0, loose: 0 }
    ), [inventoryData]);
                    ...
                    <CardHeader className="pb-3">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                            <CardTitle className="flex items-center gap-2 text-base">
                                ...
                                {t('المخزون حسب المستودعات', 'Stock by Warehouse')}
                                {!loading && (
                                                                        ...
                                                                        <>
                                                                            ...
                                                                            <div className="inline-flex items-center gap-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg px-4 py-2">
                                                                                ...
                                                                                <span className="font-mono font-bold text-amber-700 dark:text-amber-300">
                                                                                    {(record.loose_stock || 0).toLocaleString('en-US', { maximumFractionDigits: 1 })}
                                                                                </span>
                                                                                <span className="text-xs text-amber-500">{unitLabel[language === 'ar' ? 'ar' : 'en']}</span>
                                                                            </div>
                                                                        </>
                                                                    ) : (
                                                                        <p className="text-gray-400">{t('لا توجد رولونات', 'No rolls found')}</p>
                                                                    )}
                                                                </div>
                                                            ) : (
                                                                <>
