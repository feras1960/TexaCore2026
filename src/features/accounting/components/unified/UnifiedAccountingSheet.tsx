...
export function UnifiedAccountingSheet({
    ...
    const [activeTab, setActiveTab] = useState(() => {
        ...
        if (initialMode === 'create' || initialMode === 'edit') {
            const modeTab = config.tabs.find(t => t.showInModes?.includes(initialMode));
            if (modeTab) return modeTab.id;
        }
        return defaultTab || config.defaultTab;
    });
    ...
    useEffect(() => {
        if (initialData) {
            setData((prev: any) => {
                ...
                // ═══ Guard: In create/edit mode, if user has typed data (name_ar, name_en, etc.)
                // but parent still sends the same skeleton without those fields,
                // skip the merge to prevent wiping user input. ═══
                if (!initialData?.id && prev) {
                    const userTypedFields = ['name_ar', 'name_en', 'name_tr', 'name_ru', 'name_uk', 'phone', 'email', 'address'];
                    const hasUserData = userTypedFields.some(f => prev[f] && !initialData[f]);
                    if (hasUserData) {
                        return prev;
                    }
                }
    ...
    // Reset tab when docType changes (NOT mode — mode changes should not reset tab)
    ...
    useEffect(() => {
        ...
        if (currentMode === 'create' || currentMode === 'edit') {
            const modeTab = config.tabs.find(t => t.showInModes?.includes(currentMode));
            if (modeTab) {
                setActiveTab(modeTab.id);
                return;
            }
        }
        setActiveTab(defaultTab || config.defaultTab);
