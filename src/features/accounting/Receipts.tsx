...
export default function Receipts() {
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    let data = entries.filter((item) => item.type === 'receipt');
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10" dir={direction}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <h1 className="text-2xl font-bold text-erp-navy dark:text-white font-cairo">{t('accounting.receipts')}</h1>
        <div className="flex items-center gap-2">
          <Button size="sm" className="bg-erp-teal hover:bg-erp-teal/90 text-white gap-1.5" onClick={() => { setDefaultTab('receipt'); setIsNewEntryOpen(true); }}>
            <Plus className="w-3.5 h-3.5" />
            {t('accounting.receipt.title') || 'إضافة مقبوضات'}
          </Button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6 px-4 py-2.5 bg-gradient-to-r from-gray-50 to-white border-b shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">{language === 'ar' ? 'عدد المقبوضات' : 'Receipts'}:</span>
