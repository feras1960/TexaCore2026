...
import {
    ...
    Store, Zap, Shield, Check,
} from 'lucide-react';
import { ContainerStatusBadge } from './ContainerStatusStepper';
...
interface UnifiedTradeSheetProps {
    ...
    /** Explicit company ID (falls back to useCompany) */
    companyId?: string | null;
    /** Callback to refresh list data after actions */
    onRefresh?: () => void;
    // ═══ Stage Awareness (NEW) ═══
    /** Current transaction stage (e.g., 'draft', 'quotation', 'order') */
    currentStage?: string;
    /** Stage transition logs for the timeline */
    stageLogs?: Array<{
        id: string;
        from_stage: string;
        to_stage: string;
...
export const UnifiedTradeSheet: React.FC<UnifiedTradeSheetProps> = ({
    ...
    const TypeSelectorHeader = hasHeaderContent ? (
        <div className="flex flex-col w-full">
            <div className="flex items-center justify-between w-full px-4 py-1.5 border-b bg-gray-50/80 dark:bg-gray-800/80 min-h-[46px]">
                <div className="flex items-center gap-2">
                    {allowTypeSwitch && mode === 'sales' ? (
                        <>
            ...
            {/* Stage Timeline */}
            {(currentStage && !isCreateMode) && (
                <div className="mt-2">
                    <TransactionStageTimeline
                        type={mode === 'sales' ? 'sale' : 'purchase'}
                        currentStage={currentStage}
                        stageHistory={stageLogs?.map(l => ({
                            id: l.id,
                            transaction_type: (mode === 'sales' ? 'sale' : 'purchase') as 'purchase' | 'sale',
                            transaction_id: '',
                            from_stage: l.from_stage,
                            to_stage: l.to_stage,
                            generated_number: l.generated_number || null,
                            notes: l.notes || null,
                            metadata: {},
                            performed_by: null,
                            performed_by_name: l.performed_by_name || null,
            ...
            {/* Flow indicator (desktop only, hidden in create mode) — legacy fallback */}
            {
                !isCreateMode && !currentStage && (
