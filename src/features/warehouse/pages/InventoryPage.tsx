...
export default function InventoryPage() {
    ...
    // Fetch warehouse data for a material (cached, with optional force refresh)
    const fetchWarehouseData = useCallback(async (materialId: string, matOrForce?: InventoryMaterialRow | true) => {
        ...
        if (!isForce && whCache[materialId]) return; // already cached
        if (!companyId) return;

        // Find the material data for loose stock fallback
        ...
        setWhLoading(p => new Set(p).add(materialId));
        try {
            let rows = await warehouseService.getMaterialStockByWarehouse(companyId, materialId);
            // Client-side fallback: if service returned empty but we know there's loose stock
            if (rows.length === 0 && looseStock > 0) {
                ...
                if (wh) {
                    rows = [{
                        ...
                        warehouse_name_ar: wh.name_ar || '', warehouse_name_en: wh.name_en || '',
                ...
                if (!hasLoose) rows[0] = { ...rows[0], loose_stock: looseStock };
            }
            setWhCache(p => ({ ...p, [materialId]: rows }));
        } catch { setWhCache(p => ({ ...p, [materialId]: [] })); }
        finally { setWhLoading(p => { const n = new Set(p); n.delete(materialId); return n; }); }
    }, [whCache, companyId, materials]);

    // Toggle material expand
    const toggleExpand = useCallback((id: string, mat: InventoryMaterialRow) => {
        setExpandedId(prev => {
            const newVal = prev === id ? null : id;
            if (newVal) fetchWarehouseData(id, mat);
            return newVal;
        });
    }, [fetchWarehouseData]);

    // Fetch rolls for a warehouse (cached, with optional force refresh)
    const fetchRollData = useCallback(async (materialId: string, warehouseId: string, force?: boolean) => {
        const whKey = `${materialId}::${warehouseId}`;
        if (!force && rollCache[whKey]) return;
        if (!companyId) return;
        setRollLoading(p => new Set(p).add(whKey));
        try {
            const rolls = await warehouseService.getMaterialRollsDetail(companyId, materialId, warehouseId);
            setRollCache(p => ({ ...p, [whKey]: rolls }));
        } catch { setRollCache(p => ({ ...p, [whKey]: [] })); }
        finally { setRollLoading(p => { const n = new Set(p); n.delete(whKey); return n; }); }
    }, [rollCache, companyId]);
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on('postgres_changes', {
                ...
                // Also refresh main inventory data
                refetch();
            })
            .on('postgres_changes', {
                event: 'UPDATE',
                ...
                const materialId = payload.new?.id;
                if (!materialId) return;
                setWhCache(p => { const n = { ...p }; delete n[materialId]; return n; });
                if (expandedIdRef.current === materialId) {
                    setTimeout(() => fetchWarehouseData(materialId, true), 300);
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                                {fmt0(summary.totalMaterials)} {t('warehouse.tabs.materials')} · {fmt0(summary.totalRolls)} {isRTL ? 'رولون' : 'rolls'} · {fmt(summary.totalMeters + summary.totalLooseStock)} {t('inventory.meterUnit')}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-end gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex-shrink-0 flex-wrap">
                        ...
                        {filterOptions.colors.length > 0 && (
                            <FilterSelect
                                ...
                                options={[
                                    { value: 'all', label: isRTL ? 'كل الألوان' : 'All Colors' },
                        ...
                        {/* Stock Type */}
                        <FilterSelect
                            label={isRTL ? 'نوع المخزون' : 'Stock Type'}
                            value={filters.stockType}
                            onChange={v => setFilters(p => ({ ...p, stockType: v as any }))}
                            options={[
                                { value: 'all', label: isRTL ? '📦 الكل' : '📦 All' },
                                { value: 'rolls', label: isRTL ? '🎯 رولونات فقط' : '🎯 Rolls Only' },
                                { value: 'loose', label: isRTL ? '📏 سائب فقط' : '📏 Loose Only' },
                                        ...
                                        return (
                                            <React.Fragment key={mat.material_id}>
                                                ...
                                                >
                                                    ...
                                                    {/* Material name — click opens sheet */}
                                                            ...
                                                            {costVal > 0 ? (
                                                                <div className="flex flex-col items-end gap-0">
                                                                    ...
                                                                    <p className="text-[9px] text-gray-400 leading-tight mt-0.5">
                                                                        {mat.avg_cost_per_meter > 0 ? t('inventory.avgCost') : (isRTL ? 'سعر الشراء' : 'Purchase')}
                                                                    </p>
                                                                </div>
