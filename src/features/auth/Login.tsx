...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (errorParam === 'tenant_deleted') {
      setFormError(
        language === 'ar'
          ? '⚠️ تم حذف أو تعليق اشتراكك. الرجاء التواصل مع الدعم الفني.'
          : '⚠️ Your subscription has been deleted or suspended. Please contact support.'
      );
      window.history.replaceState({}, '', '/login');
    } else if (errorParam === 'account_removed') {
      setFormError(
        language === 'ar'
          ? '⚠️ تم إلغاء حسابك من النظام. إذا كنت تعتقد أن هذا خطأ، يرجى التواصل مع الإدارة.'
          : '⚠️ Your account has been removed from the system. If you believe this is an error, please contact administration.'
      );
      window.history.replaceState({}, '', '/login');
    }
  }, [language]);
