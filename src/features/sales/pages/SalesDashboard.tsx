  ...
  return (
    <div className="bg-white dark:bg-gray-900 shadow-soft-md rounded-lg px-3 py-2 border border-gray-100 dark:border-gray-700">
      ...
      <p className="font-mono font-bold text-emerald-600 text-sm">
        {sym} {Number(payload[0].value).toLocaleString()}
      </p>
      {payload[0].payload?.count > 0 && (
        <p className="text-[11px] text-gray-400 font-tajawal">
          {payload[0].payload.count} فاتورة
        </p>
      )}
    </div>
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: txs } = await supabase
        ...
        .select('id, stage, customer_id, customer_name, currency, total_amount, created_at')
        .eq('company_id', companyId);
      const allRows = txs || [];
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-1">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select value={selectedCurrency} onValueChange={setSelectedCurrency}>
            <SelectTrigger className="w-[175px] h-10 text-sm border-gray-200 dark:border-gray-700 hover:border-emerald-500 transition-colors">
              <Coins className="w-4 h-4 me-2 text-emerald-500" />
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthly} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <Area type="monotone" dataKey="total" stroke="#10B981" strokeWidth={2.5} fillOpacity={1} fill="url(#salesGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
